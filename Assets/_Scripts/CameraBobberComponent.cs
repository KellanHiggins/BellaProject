...
public class CameraBobberComponent : MonoBehaviour {
	...
	private PlayerControlledComponent characterMotor; // we need to see if the character is walking or jumping (TODO maybe running and sliding)
...
//		heightDependency = transform.localScale.y * height / 2;
	...
	void Update ()
	{
		bobbingAmount = 10f;// bobbingRatio;// * heightDependency;
		...
		else 
		{
			...
			if( (stateMask & isWalking) > 0 )
			{ // if we got here, we're stopping
		...
		Camera.main.transform.Translate(Vector3.up * bobbingDelta * 10f, Space.World);
	}
	...
	private void UpdatePhaseAndBobbingDelta ( float direction )
	{
		...
		currentBobbing = (Mathf.Cos(phase) + 1.0f) * 10f;
